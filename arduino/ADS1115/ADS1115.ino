...
class MUX {
  ...
  void start_pca ()
  {
    TroykaI2CHub hub = this->hub;
    Adafruit_MCP4725* dac = this->dac;
    hub.begin();
    for(int i = 0; i < 8; i++)
    {
      hub.setBusChannel(i);
      //this->status[i] = this->dac[i].begin();
      Serial.println(dac[i].begin(adr_pca[0]));
    }
  }
  ...
  private:
    uint8_t adr_hub;
    bool status[SIZE]; //connect or not connect
    float result[SIZE]; //output voltage of DAC
    Adafruit_MCP4725 dac[SIZE];
    TroykaI2CHub hub;
  };
///////////////////////////
...
// здесь пробегаем массив мультиплексоров
